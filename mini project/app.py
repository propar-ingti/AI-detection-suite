        ...
        with c2:
            ...
            if st.button("🔍 Scan Pixels"):
                with st.spinner('Deconstructing image layers...'):
                    ...
                    st.session_state.scan_results = image.predict_image(temp_path)
                    
                    if os.path.exists(temp_path):
                        os.remove(temp_path)
            
            # Render results outside the button block if they exist
            if st.session_state.scan_results is not None:
                res = st.session_state.scan_results
                
                st.write(f"### {res['Verdict']}")
                
                # Progress bar expects a float between 0.0 and 1.0
                st.progress(res['Fake Probability'] / 100, text="AI Probability Indicator")
                
                if res['Fake Probability'] > 50:
                    st.error(f"High risk detected: {res['Fake Probability']}% AI Generated")
                else:
                ...
                st.markdown(
                    ...
                    [data-testid="stMetricLabel"] {
                        color: #A3AED0 !important;  /* Soft light blue/grey label */
                    }
                    </style>
                    """,
                    unsafe_allow_html=True
                ...
                # Added the '%' symbol to make the metric clear
